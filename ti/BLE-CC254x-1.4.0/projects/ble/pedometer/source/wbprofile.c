...
// Simple Profile Characteristic 5 Properties, Value, User Description
...
// Simple Profile Characteristic 6 Properties, Value, User Description
static uint8		simpleProfileChar6Props = GATT_PROP_READ | GATT_PROP_WRITE;
static uint8		simpleProfileChar6[SIMPLEPROFILE_CHAR6_LEN] = { 0, 0, 0 };
...
bStatus_t SimpleProfile_SetParameter(uint8 param, uint8 len, void *value)
{
	...
	case SIMPLEPROFILE_CHAR5:
		if (len == SIMPLEPROFILE_CHAR5_LEN) {
			osal_memcpy(simpleProfileChar5, value, SIMPLEPROFILE_CHAR5_LEN);
		} else {
			ret = bleInvalidRange;
		}
		break;
	...
	case SIMPLEPROFILE_CHAR6:
		if (len == SIMPLEPROFILE_CHAR6_LEN) {
			osal_memcpy(simpleProfileChar6, value, SIMPLEPROFILE_CHAR6_LEN);
		} else {
			ret = bleInvalidRange;
		}
		break;
...
bStatus_t SimpleProfile_GetParameter(uint8 param, void *value)
{
	...
	case SIMPLEPROFILE_CHAR5:
		osal_memcpy(value, simpleProfileChar5, SIMPLEPROFILE_CHAR5_LEN);
		break;

	case SIMPLEPROFILE_CHAR6:
		osal_memcpy(value, simpleProfileChar6, SIMPLEPROFILE_CHAR6_LEN);
		break;
	...
	if (pAttr->type.len == ATT_BT_UUID_SIZE) {
		...
		case SIMPLEPROFILE_CHAR5_UUID:
			*pLen = SIMPLEPROFILE_CHAR5_LEN;
			osal_memcpy(pValue, pAttr->pValue, SIMPLEPROFILE_CHAR5_LEN);
			break;

		case SIMPLEPROFILE_CHAR6_UUID:
			*pLen = SIMPLEPROFILE_CHAR6_LEN;
			osal_memcpy(pValue, pAttr->pValue, SIMPLEPROFILE_CHAR6_LEN);
			break;
...
static bStatus_t simpleProfile_WriteAttrCB(uint16 connHandle, gattAttribute_t *pAttr, uint8 *pValue, uint8 len, uint16 offset)
{
	...
	if (pAttr->type.len == ATT_BT_UUID_SIZE) {
		...
		case SIMPLEPROFILE_CHAR6_UUID:
			...
			if (status == SUCCESS) {
				osal_memcpy(pAttr->pValue, pValue, SIMPLEPROFILE_CHAR6_LEN);
